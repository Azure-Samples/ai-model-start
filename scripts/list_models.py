...
Non-OpenAI models (DeepSeek, Meta, xAI, etc.) that support the Responses API
have "agentsV2" in their capabilities instead. Use --non-openai to list these.

Usage:
    python list_models.py                          # OpenAI models with responses
    python list_models.py --non-openai             # non-OpenAI models (agentsV2)
    python list_models.py --agents                 # all models with agentsV2
...
def list_responses_models(subscription_id=None, show_locations=False, non_openai=False, agents=False):
    ...
    if agents:
        label = "models with agents (agentsV2) support"
    elif non_openai:
        label = "non-OpenAI models with Responses API support (agentsV2)"
    else:
    ...
    for location in LOCATIONS:
        try:
            for entry in client.models.list(location=location):
                ...
                if agents:
                    # Show all models (any provider) with agentsV2 support.
                    if caps.get("agentsV2") != "true":
                        continue
                elif non_openai:
                    # Show non-OpenAI models with agentsV2 support.
                    # These work with the Responses API even though the ARM
                    # capabilities don't tag them with "responses".
                    if fmt == "OpenAI":
                        continue
                    if caps.get("agentsV2") != "true":
                        continue
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="non_openai",
        help="List non-OpenAI models (DeepSeek, Meta, xAI, etc.) with agentsV2 support (proxy for Responses API)",
    )
    parser.add_argument(
        "--agents",
        action="store_true",
        help="List all models (any provider) with agentsV2 capability",
    )
    args = parser.parse_args()
    list_responses_models(args.subscription, args.locations, args.non_openai, args.agents)
